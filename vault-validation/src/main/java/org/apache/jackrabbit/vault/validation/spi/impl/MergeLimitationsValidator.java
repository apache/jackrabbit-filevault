...
public class MergeLimitationsValidator implements DocumentViewXmlValidator {
    ...
    protected static final String PACKAGE_NON_ROOT_NODE_MERGED =
            "Non-empty Node '%s' is supposed to be imported with mode 'merge' but it is not the aggregator's root node. This is currently not supported by FileVault (https://issues.apache.org/jira/browse/JCRVLT-255).";
    ...
    public Collection<ValidationMessage> validate(
        ...
        if (!isRoot
                ...
                && rootNodePathsOfMergeRules.contains(nodeContext.getNodePath())) {
            return Collections.singleton(new ValidationMessage(
                    severity, String.format(Locale.ENGLISH, PACKAGE_NON_ROOT_NODE_MERGED, nodeContext.getNodePath())));
        }
        return null;
    }
}
