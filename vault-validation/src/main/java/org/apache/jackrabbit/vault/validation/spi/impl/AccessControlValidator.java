...
public class AccessControlValidator implements DocumentViewXmlValidator {
    ...
    protected static final String MESSAGE_IGNORED_ACCESS_CONTROL_LIST =
            "Found an access control list, but it is never considered during installation as the property 'acHandling' is set to '%s'!";
    protected static final String MESSAGE_INEFFECTIVE_ACCESS_CONTROL_LIST =
            "Found no access control list, but there is supposed to be one contained as the property 'acHandling' is set to '%s'!";
    ...
    public AccessControlValidator(
            boolean isIncremental, ValidationMessageSeverity severity, AccessControlHandling accessControlHandling) {
        super();
    ...
    public Collection<ValidationMessage> done() {
        if (!isIncremental) {
            ...
            if (!hasFoundACLNode
                    && accessControlHandling != AccessControlHandling.IGNORE
                    && accessControlHandling != AccessControlHandling.CLEAR) {
                return Collections.singleton(new ValidationMessage(
                        severity,
                        String.format(Locale.ENGLISH, MESSAGE_INEFFECTIVE_ACCESS_CONTROL_LIST, accessControlHandling)));
            }
        }
        return null;
    }
    ...
    public @Nullable Collection<ValidationMessage> validate(
        ...
        if (ACL_MANAGEMENT.isACLNodeType(node.getPrimaryType().orElse(""))) {
            ...
            if (accessControlHandling == AccessControlHandling.IGNORE
                ...
                return Collections.singleton(new ValidationMessage(
                        severity,
                        String.format(Locale.ENGLISH, MESSAGE_IGNORED_ACCESS_CONTROL_LIST, accessControlHandling)));
            }
        }
        return null;
    }
