...
/**
 * {@code ExampleHookService}...
 */
@Component(
        service = InstallHook.class,
        immediate = true,
        property = {"service.vendor=The Apache Software Foundation"}
)
public class ExampleHookService implements InstallHook {

    private static String PROP_COPY_FROM = "hook-example-copyfrom";

    private static String PROP_COPY_TO = "hook-example-copyto";

    private static String PROP_TEST_NODE = "hook-example-testnode";
     ...
     * default logger
     */
    private static final Logger log = LoggerFactory.getLogger(ExampleHookService.class);
    ...
    private void activate() {
        log.info("*** activated example install hook ***");
    }

    @Deactivate
    private void deactivate() {
        log.info("*** deactivated example install hook ***");
    }
    ...
    private void doPrepare(InstallContext ctx) throws PackageException, RepositoryException {
        // as and example, dump all the installed packages
        JcrPackageManager mgr = packaging.getPackageManager(ctx.getSession());
        for (JcrPackage pkg: mgr.listPackages()) {
            log.info("hook-example; installed package: {}", pkg.getDefinition().getId());
        }
