...
public class AdvancedPropertiesValidatorTest {
    ...
    public void testProperties()
        ...
        Mockito.when(propertiesValidator1.validate(Mockito.any()))
        ...
        try (InputStream input = this.getClass().getResourceAsStream("/simple-package/META-INF/vault/properties.xml");
                InputStream input2 =
                        this.getClass().getResourceAsStream("/simple-package/META-INF/vault/properties.xml")) {
            Collection<ValidationMessage> messages =
                    validator.validateMetaInfData(input, Paths.get("vault/properties.xml"));
            ValidationExecutorTest.assertViolation(
                    messages, new ValidationViolation("id1", ValidationMessageSeverity.ERROR, "error1"));
    ...
    public void testPropertiesWithInvalidElement()
        ...
        try (InputStream input =
                this.getClass().getResourceAsStream("/invalid-package/META-INF/vault/properties.xml")) {
            Collection<ValidationMessage> messages =
                    validator.validateMetaInfData(input, Paths.get("vault/properties.xml"));
            ValidationExecutorTest.assertViolation(
                    messages,
                    new ValidationMessage(
                            ...
                            new InvalidPropertiesFormatException(
                                    "org.xml.sax.SAXParseException; lineNumber: 35; columnNumber: 19; Element type \"someinvalidentry\" must be declared.")));
        }
    }
}
