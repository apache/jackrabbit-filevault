...
public final class EffectiveNodeType {
    ...
    public static @NotNull EffectiveNodeType ofPrimaryTypeAndMixins(
    ...
    private EffectiveNodeType(@NotNull List<NodeType> nodeTypes) {
        ...
        propertyDefinitions = CachingSupplier.of(() -> nodeTypes.stream()
                .flatMap(nt -> Arrays.stream(nt.getPropertyDefinitions()))
                .collect(Collectors.toList()));
        nodeDefinitions = CachingSupplier.of(() -> nodeTypes.stream()
                .flatMap(nt -> Arrays.stream(nt.getChildNodeDefinitions()))
                .collect(Collectors.toList()));
    }
     ...
     *
     */
    public Optional<PropertyDefinition> getApplicablePropertyDefinition(
            @NotNull String name, boolean isMultiple, int type) {
        // This replicates the logic from
        // https://github.com/apache/jackrabbit-oak/blob/274f92402a12978040939965e92ee4519f2ce1c3/oak-core/src/main/java/org/apache/jackrabbit/oak/plugins/nodetype/EffectiveNodeTypeImpl.java#L365
        // single values are covered by multivalue property definitions as well (but not vice-versa)
        return getApplicablePropertyDefinition(
                pd -> ((!isMultiple) || isMultiple == pd.isMultiple())
                        && (type == pd.getRequiredType() || UNDEFINED == type || UNDEFINED == pd.getRequiredType()),
                name);
    }

    public Optional<PropertyDefinition> getApplicablePropertyDefinition(
            Predicate<PropertyDefinition> predicate, @NotNull String name) {
        // first named then unnamed
        Optional<PropertyDefinition> namedPropertyDef =
                EffectiveNodeType.<PropertyDefinition>getApplicableItemDefinition(
                        propertyDefinitions.get(), predicate, name);
        if (!namedPropertyDef.isPresent()) {
            // then unnamed
            return EffectiveNodeType.<PropertyDefinition>getApplicableItemDefinition(
                    propertyDefinitions.get(), predicate, null);
        } else {
    ...
    public Optional<NodeDefinition> getApplicableChildNodeDefinition(@NotNull String name, @NotNull NodeType... types) {
        // This replicates the logic from
        // https://github.com/apache/jackrabbit-oak/blob/274f92402a12978040939965e92ee4519f2ce1c3/oak-core/src/main/java/org/apache/jackrabbit/oak/plugins/nodetype/EffectiveNodeTypeImpl.java#L440
        return getApplicableChildNodeDefinition(
                nd -> Arrays.stream(nd.getRequiredPrimaryTypeNames())
                        .allMatch(requiredPrimaryType -> Arrays.stream(types)
                                .anyMatch(providedType -> providedType.isNodeType(requiredPrimaryType))),
                name);
    }

    public Optional<NodeDefinition> getApplicableChildNodeDefinition(
            @NotNull Predicate<NodeDefinition> predicate, @NotNull String name) {
        // first named then unnamed
        Optional<NodeDefinition> namedNodeDef =
                EffectiveNodeType.<NodeDefinition>getApplicableItemDefinition(nodeDefinitions.get(), predicate, name);
        if (!namedNodeDef.isPresent()) {
            // then unnamed
            return EffectiveNodeType.<NodeDefinition>getApplicableItemDefinition(
                    nodeDefinitions.get(), predicate, null);
        } else {
            return namedNodeDef;
        }
    }

    private static <T extends ItemDefinition> Optional<T> getApplicableItemDefinition(
            List<T> itemDefinitions, Predicate<T> predicate, @Nullable String name) {
     ...
     */
    public Optional<String> getDefaultPrimaryChildNodeTypeName(@NotNull Node parent, @NotNull String nodeName)
            throws RepositoryException {
        Optional<NodeDefinition> nodeDefinition =
                getApplicableChildNodeDefinition(nd -> nd.getDefaultPrimaryType() != null, nodeName);
