...
public class SubPackageFilterArchiveTest {
    ...
    public void testGetEntry() throws IOException {
        assertNotNull(
                "/etc/packages must not be null in memory archive", memoryArchive.getEntry("/jcr_root/etc/packages"));
        assertNotNull(
                "/etc/packages must not be null in memory archive",
                memoryArchive.getEntry("/jcr_root/etc/packages/my_packages/sub_a.zip"));
