...
1. Create or update a `RELEASE-NOTES.txt` file in the root folder of the project to be released. When done, commit the
   file directly to the master branch. See previous release notes for examples of what to include. The release note report in [Jira][3] is a useful
...
1. Upload the artifacts to https://dist.apache.org/repos/dist/dev/jackrabbit/filevault via SVN
        ...
        # change into a different directory, don't do this in your local git checkout of filevault 
...
1. Start the vote thread, wait **72 hours**. Use the `vote.txt` template generated by the Maven build in `target/checkout/target` as
   template. Fill out the missing information.
