...
public class ACEsAtRootIT extends IntegrationTestBase {
    ...
    public void export() throws RepositoryException, IOException, PackageException, ConfigurationException {
        ...
        AccessControlUtils.addAccessControlEntry(
                admin, null, principal1, new String[] {"jcr:namespaceManagement"}, true);
        AccessControlUtils.addAccessControlEntry(admin, "/", principal1, new String[] {"jcr:read"}, true);

        // Check the number of ACEs at the root
        AccessControlEntry[] originalAces =
                AccessControlUtils.getAccessControlList(admin, "/").getAccessControlEntries();
        ...
        assertNodeExists(userPath);
        AccessControlEntry[] actualAces =
                AccessControlUtils.getAccessControlList(admin, "/").getAccessControlEntries();
        assertEquals(
                "Expected two ACEs: " + toString(originalAces) + " at / but found: " + toString(actualAces),
                2,
                actualAces.length);
        assertEquals(1, AccessControlUtils.getAccessControlList(admin, null).getAccessControlEntries().length);
    ...
    static String toString(AccessControlEntry[] aces) {
        ...
        for (AccessControlEntry ace : aces) {
            sb.append("ACE for principal ")
                    .append(ace.getPrincipal().getName())
                    .append(" with privileges ")
                    .append(StringUtils.join(ace.getPrivileges(), ","))
                    .append("\n");
        }
        return sb.toString();
    }
