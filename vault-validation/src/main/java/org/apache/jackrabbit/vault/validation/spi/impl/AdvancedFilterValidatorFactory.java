...
public final class AdvancedFilterValidatorFactory implements ValidatorFactory {
    ...
    @Deprecated
    private static final Object OPTION_SEVERITY_FOR_UNCOVERED_FILTER_ROOT_ANCESTORS =
            "severityForUncoveredFilterRootAncestors"; // TODO: confusing naming

    private static final Object OPTION_SEVERITY_FOR_UNDEFINED_FILTER_ROOT_ANCESTORS =
            "severityForUndefinedFilterRootAncestors";
    ...
    static final ValidationMessageSeverity DEFAULT_SEVERITY_FOR_UNDEFINED_FILTER_ROOT_ANCESTORS =
            ValidationMessageSeverity.WARN;
    static final ValidationMessageSeverity DEFAULT_SEVERITY_FOR_ORPHANED_FILTER_RULES = ValidationMessageSeverity.WARN;
    static final Collection<String> DEFAULT_VALID_ROOTS =
            new LinkedList<>(Arrays.asList("/", "/libs", "/apps", "/etc", "/var", "/tmp", "/content", "/etc/packages"));
    ...
    public Validator createValidator(@NotNull ValidationContext context, @NotNull ValidatorSettings settings) {
        ...
        if (settings.getOptions().containsKey(OPTION_SEVERITY_FOR_UNDEFINED_FILTER_ROOT_ANCESTORS)) {
            ...
            // application packages must define every ancestor via package dependencies according to
            ...
            if (PackageType.APPLICATION.equals(context.getProperties().getPackageType())) {
                ...
                log.warn("Using deprecated option " + OPTION_SEVERITY_FOR_UNCOVERED_FILTER_ROOT_ANCESTORS
                        + ". Please switch to " + OPTION_SEVERITY_FOR_UNDEFINED_FILTER_ROOT_ANCESTORS + " instead!");
        ...
        return new AdvancedFilterValidator(
                factory,
                settings.getDefaultSeverity(),
                severityForUncoveredAncestorNode,
                severityForUndefinedFilterRootAncestors,
                severityForOrphanedFilterRules,
                context.getContainerValidationContext() != null,
                context.getDependenciesPackageInfo(),
                context.getFilter(),
                validRoots);
    }
