...
public class QNameComparator implements Comparator<QName> {
    ...
    public int compare(final QName o1, final QName o2) {
        ...
        if (!o1.getPrefix().equalsIgnoreCase(XMLConstants.DEFAULT_NS_PREFIX)
                && o2.getPrefix().equalsIgnoreCase(XMLConstants.DEFAULT_NS_PREFIX)) {
            return LESS_THAN;
        } else if (o1.getPrefix().equalsIgnoreCase(XMLConstants.DEFAULT_NS_PREFIX)
                && !o2.getPrefix().equalsIgnoreCase(XMLConstants.DEFAULT_NS_PREFIX)) {
            return GREATER_THAN;
        } else {
            // order first by prefix, then by local name
            String lowerCaseQName1 =
                    o1.getPrefix().toLowerCase(Locale.ROOT) + o1.getLocalPart().toLowerCase(Locale.ROOT);
            String lowerCaseQName2 =
                    o2.getPrefix().toLowerCase(Locale.ROOT) + o2.getLocalPart().toLowerCase(Locale.ROOT);
