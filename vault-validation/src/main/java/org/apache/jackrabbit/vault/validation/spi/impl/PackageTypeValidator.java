...
public final class PackageTypeValidator
    ...
    protected static final String MESSAGE_FILTER_HAS_INCLUDE_EXCLUDES =
            "Package of type '%s' is not supposed to contain includes/excludes below any of its filters!";
    protected static final String MESSAGE_FILTER_CONTAINS_MUTABLE_ROOTS =
            "Package of type '%s' is not supposed to contain mutable filter root paths but has filter root '%s'!";
    protected static final String MESSAGE_FILTER_CONTAINS_MUTABLE_ROOTS_OUTSIDE_PACKAGES =
            "Package of type '%s' is not supposed to contain mutable filter root paths except for subpackages below in \"/etc/packages/\" but has filter root '%s'!";
    protected static final String MESSAGE_FILTER_CONTAINS_IMMUTABLE_ROOTS =
            "Package of type '%s' is not supposed to contain immutable filter root paths but has filter root '%s'!";
    protected static final String MESSAGE_UNSUPPORTED_SUB_PACKAGE_OF_TYPE =
            "Package of type '%s' must only contain sub packages of type '%s' but found subpackage of type '%s'!";
    protected static final String MESSAGE_UNSUPPORTED_SUB_PACKAGE =
            "Package of type '%s' is not supposed to contain any subpackages!";
    protected static final String MESSAGE_DEPENDENCY =
            "Package of type '%s' must not have package dependencies but found dependencies '%s'!";
    protected static final String MESSAGE_LEGACY_TYPE =
            "Package of type '%s' is legacy. Use one of the other types instead!";
    protected static final String MESSAGE_PACKAGE_HOOKS =
            "Package of type '%s' must not contain package hooks but has '%s'!";
    protected static final String MESSAGE_NO_PACKAGE_TYPE_SET =
            "No package type set, make sure that property 'packageType' is set in the properties.xml!";
    protected static final String MESSAGE_NO_OSGI_BUNDLE_OR_CONFIG_ALLOWED =
            "Package of type '%s' is not supposed to contain OSGi bundles or configurations!";
    protected static final String MESSAGE_ONLY_OSGI_BUNDLE_OR_CONFIG_OR_SUBPACKAGE_ALLOWED =
            "Package of type '%s' is not supposed to contain anything but OSGi bundles/configurations and subpackages!";
    protected static final String MESSAGE_APP_CONTENT =
            "Package of type '%s' is not supposed to contain content below root nodes %s!";
    protected static final String MESSAGE_NO_APP_CONTENT_FOUND =
            "Package of type '%s' is not supposed to contain content outside root nodes %s!";
    protected static final String MESSAGE_PROHIBITED_MUTABLE_PACKAGE_TYPE =
            "All mutable package types are prohibited and this package is of mutable type '%s'";
    protected static final String MESSAGE_PROHIBITED_IMMUTABLE_PACKAGE_TYPE =
            "All immutable package types are prohibited and this package is of immutable type '%s'";
    ...
    public PackageTypeValidator(
            @NotNull WorkspaceFilter workspaceFilter,
            @NotNull ValidationMessageSeverity severity,
            @NotNull ValidationMessageSeverity severityForNoPackageType,
            @NotNull ValidationMessageSeverity severityForLegacyType,
            boolean prohibitMutableContent,
            boolean prohibitImmutableContent,
            boolean allowComplexFilterRulesInApplicationPackages,
            boolean allowInstallHooksInApplicationPackages,
            @NotNull PackageType type,
            @NotNull Pattern jcrInstallerNodePathRegex,
            Pattern jcrInstallerAdditionalFileNodePathRegex,
            @NotNull Set<String> immutableRootNodeNames,
    ...
    boolean isImmutableContent(String nodePath) {
        return immutableRootNodeNames.stream()
                .anyMatch(rootNodeName ->
                        ("/" + rootNodeName).equals(nodePath) || nodePath.startsWith("/" + rootNodeName + "/"));
    }

    @Override
    public @Nullable Collection<ValidationMessage> done() {
        // check if questionable nodes are parents of valid nodes
        List<NodeContext> invalidNodes = potentiallyDisallowedContainerNodes.stream()
                .filter(s -> validContainerNodePaths.stream()
                        .noneMatch(p -> p.startsWith(s.getNodePath() + "/") || p.equals(s.getNodePath())))
                .collect(Collectors.toList());
        if (!invalidNodes.isEmpty()) {
            return invalidNodes.stream()
                    .map(e -> new ValidationMessage(
                            severity,
                            String.format(
                                    Locale.ENGLISH, MESSAGE_ONLY_OSGI_BUNDLE_OR_CONFIG_OR_SUBPACKAGE_ALLOWED, type),
                            e.getNodePath(),
                            e.getFilePath(),
                            e.getBasePath(),
                            null))
        ...
        switch (type) {
                ...
                if (isImmutableContent(nodeContext.getNodePath())) {
                    messages.add(new ValidationMessage(
                            ...
                            String.format(
                                    ...
                                    MESSAGE_APP_CONTENT,
                                    type,
                                    immutableRootNodeNames.stream().collect(Collectors.joining("' or '", "'", "'")))));
                }
                if (isOsgiBundleOrConfigurationNode(nodeContext.getNodePath(), true)) {
                    messages.add(new ValidationMessage(
                            severity, String.format(Locale.ENGLISH, MESSAGE_NO_OSGI_BUNDLE_OR_CONFIG_ALLOWED, type)));
                }
                break;
            case APPLICATION:
                if (!isImmutableContent(nodeContext.getNodePath())) {
            ...
            case CONTAINER:
                ...
                if (isOsgiBundleOrConfigurationNode(nodeContext.getNodePath(), true)) {
                    ...
                    // only potentially disallowed, as the node may be a parent of a sub package or osgi bundle, which
                    // is allowed as well
        ...
        switch (type) {
            case APPLICATION:
                if (!allowComplexFilterRulesInApplicationPackages && hasIncludesOrExcludes(filter)) {
                    messages.add(new ValidationMessage(
                            severity, String.format(Locale.ENGLISH, MESSAGE_FILTER_HAS_INCLUDE_EXCLUDES, type)));
                }
                getConflictingFilterRoots(filter, false)
                        .forEach(root -> messages.add(new ValidationMessage(
                                severity,
                                String.format(Locale.ENGLISH, MESSAGE_FILTER_CONTAINS_MUTABLE_ROOTS, type, root))));
                break;
            case CONTENT:
                getConflictingFilterRoots(filter, true)
                        .forEach(root -> messages.add(new ValidationMessage(
                                severity,
                                String.format(Locale.ENGLISH, MESSAGE_FILTER_CONTAINS_IMMUTABLE_ROOTS, type, root))));
                break;
            case CONTAINER:
                getConflictingFilterRoots(filter, false).stream()
                        .filter(p -> !p.startsWith("/etc/packages/"))
                        .forEach(root -> messages.add(new ValidationMessage(
                                severity,
                                String.format(
                                        Locale.ENGLISH,
                                        MESSAGE_FILTER_CONTAINS_MUTABLE_ROOTS_OUTSIDE_PACKAGES,
                                        type,
                                        root))));
                break;
            case MIXED:
    ...
    Collection<String> getConflictingFilterRoots(WorkspaceFilter filter, boolean isMutable) {
        return filter.getFilterSets().stream()
                .map(PathFilterSet::getRoot)
                .filter(root -> isImmutableContent(root) == isMutable)
                .collect(Collectors.toList());
    }
    ...
    public Collection<ValidationMessage> validate(@NotNull PackageProperties properties) {
        ...
        if (containerValidationContext != null) {
            ...
            ValidationMessage message = validateSubPackageType(
                    properties.getPackageType(),
        ...
        switch (packageType) {
            ...
            case CONTENT:
                if (prohibitMutableContent) {
                    messages.add(new ValidationMessage(
                            ...
                            String.format(
                                    Locale.ENGLISH,
                                    MESSAGE_PROHIBITED_MUTABLE_PACKAGE_TYPE,
                                    properties.getPackageType())));
                }
                break;
            case CONTAINER:
                ...
                if (properties.getDependencies() != null && properties.getDependencies().length > 0) {
                    messages.add(new ValidationMessage(
                            ...
                            String.format(
                                    ...
                                    MESSAGE_DEPENDENCY,
                                    properties.getPackageType(),
                                    StringUtils.join(properties.getDependencies()))));
                }
                if (prohibitImmutableContent) {
                    messages.add(new ValidationMessage(
            ...
            case MIXED:
                messages.add(new ValidationMessage(
                        ...
                        String.format(Locale.ENGLISH, MESSAGE_LEGACY_TYPE, properties.getPackageType())));
                if (prohibitImmutableContent) {
                    messages.add(new ValidationMessage(
                            ValidationMessageSeverity.ERROR,
                            String.format(
        ...
        switch (type) {
            ...
            case APPLICATION:
                ...
                if (node.getPrimaryType().isPresent()
                        && NODETYPE_SLING_OSGI_CONFIG.equals(
                                node.getPrimaryType().orElse(""))
                        && isOsgiBundleOrConfigurationNode(nodeContext.getNodePath(), false)) {
                    messages.add(new ValidationMessage(
                            severity, String.format(Locale.ENGLISH, MESSAGE_NO_OSGI_BUNDLE_OR_CONFIG_ALLOWED, type)));
                }
                break;
    ...
    private ValidationMessage validateSubPackageType(
            PackageType packageType, @Nullable PackageType containerPackageType) {
        ...
        switch (containerPackageType) {
            case APPLICATION:
                ...
                message = new ValidationMessage(
                        severity, String.format(Locale.ENGLISH, MESSAGE_UNSUPPORTED_SUB_PACKAGE, containerPackageType));
                break;
            case CONTENT:
                ...
                if (packageType == PackageType.MIXED) {
                    message = new ValidationMessage(
                            ...
                            String.format(
                                    ...
                                    StringUtils.join(
                                            new String[] {
                                                PackageType.APPLICATION.toString(),
                                                PackageType.CONTENT.toString(),
    ...
    @Override
    public Collection<ValidationMessage> validateMetaInfPath(
            @NotNull Path filePath, @NotNull Path basePath, boolean isFolder) {
        switch (type) {
            case APPLICATION:
                if (filePath.startsWith(PATH_HOOKS) && !allowInstallHooksInApplicationPackages)
                    ...
                    return Collections.singleton(new ValidationMessage(
                            severity, String.format(Locale.ENGLISH, MESSAGE_PACKAGE_HOOKS, type, filePath)));
            default:
                break;
        }
        return null;
