...
public class RcpTaskManagerImplTest {
    ...
    public void before() throws IOException {
        ...
        Mockito.when(mockConfigurationAdmin.getConfiguration(Mockito.anyString()))
    ...
    public void testEditTask() throws IOException, ConfigurationException, URISyntaxException {
        ...
        RcpTaskImpl taskOld = (RcpTaskImpl) taskManager.addTask(
                new RepositoryAddress("http://localhost:4502"),
                connectionOptionsBuilder.build(),
                new SimpleCredentials("testUser", "pw".toCharArray()),
                "/target/path",
                "2",
                Arrays.asList("exclude1", "exclude2"),
                false);
        RcpTaskImpl taskNew =
                (RcpTaskImpl) taskManager.editTask(taskOld.getId(), null, null, null, null, null, null, null);
    ...
    public void testSerializeDeserialize()
            throws IOException, ConfigurationException, URISyntaxException, RepositoryException {
        DefaultWorkspaceFilter filter = new DefaultWorkspaceFilter();
        try (InputStream input = this.getClass().getResourceAsStream("/filter.xml")) {
            filter.load(input);
        }
        ...
        RcpTask task1 = taskManager.addTask(
                new RepositoryAddress("http://localhost:4502"),
                connectionOptionsBuilder.build(),
                new SimpleCredentials("testUser", "pw".toCharArray()),
                "/target/path",
        ...
        taskManager.addTask(
                ...
                connectionOptionsBuilder.build(),
                new SimpleCredentials("testUser3", "pw3".toCharArray()),
                "/target/path5",
                "3",
                filter,
                true);
        ...
        // convert to Map
        RcpTaskManagerImpl taskManager2 = new RcpTaskManagerImpl(
                mockBundleContext,
                mockConfigurationAdmin,
                RcpTaskManagerImpl.createMapFromDictionary(configProperties));
        // how to get list ordered by id?
        MatcherAssert.assertThat(
                taskManager2.tasks.values(),
                IsIterableContainingInOrder.contains(taskManager.tasks.values().toArray()));
    }
}
